...
class GaussianModel:
    ...
    def construct_list_of_attributes(self):
        l = ['x', 'y', 'z', 'ar', 'ag', 'ab', 'nx', 'ny', 'nz'] # position x,y,z; albedo r,g,b; normal x,y,z
        l.append('opacity')
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, albedo, normal, opacities, scale, rotation), axis=1)
    ...
    def load_ply(self, path):
        ...
        albedos = np.stack((np.asarray(plydata.elements[0]["ar"]),
                np.asarray(plydata.elements[0]["ag"]),
                np.asarray(plydata.elements[0]["ab"])),  axis=1)
        ...
        self._albedo = nn.Parameter(torch.tensor(albedos, dtype=torch.float, device="cuda").contiguous().requires_grad_(True))
    ...
    def densification_postfix(self, new_xyz, new_albedo, new_normal, new_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
        "albedo": new_albedo,
        "normal": new_normal,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_xyz, new_albedo, new_normal, new_opacity, new_scaling, new_rotation)

        # prune_filter = torch.cat((selected_pts_mask, torch.zeros(N * selected_pts_mask.sum(), device="cuda", dtype=bool)))
        # self.prune_points(prune_filter)

    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        print("in densify and clone")
        # Extract points that satisfy the gradient condition
        ...
        self.densification_postfix(new_xyz, new_albedo, new_normal, new_opacities, new_scaling, new_rotation)
